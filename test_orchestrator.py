...
class TestYamlLikeParsing(unittest.TestCase):
    ...
    def test_skynet_profile_uses_budget_controlled_medium_effort(self) -> None:
        profiles = load_agent_profiles(Path(__file__).resolve().parent / "orchestrator" / "agents.yaml")
        skynet = profiles["skynet"]
        self.assertEqual(skynet.get("model"), "gpt-5.4")
        self.assertEqual(skynet.get("effort"), "medium")
