...
class TestProactive3hAutofixMonitorSweep(unittest.TestCase):
    ...
    def test_cancels_blocked_job_superseded_by_newer_terminal_same_role(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = Path(td) / "jobs.sqlite"
            conn = sqlite3.connect(str(db_path))
            conn.row_factory = sqlite3.Row
            ...
            # Newer terminal backend job should supersede the blocked one.
            conn.execute(
                """
                INSERT INTO jobs(
                    job_id, source, role, input_text, request_type, priority, model, effort, mode_hint,
            ...
            conn = sqlite3.connect(str(db_path))
            row = conn.execute("SELECT state, blocked_reason FROM jobs WHERE job_id='blocked-old-role'").fetchone()
            followup = conn.execute(
                "SELECT role, state FROM jobs WHERE parent_job_id=? AND job_id NOT IN ('blocked-old-role', 'done-new-role') ORDER BY created_at DESC LIMIT 1",
                (order_id,),
            ).fetchone()
            conn.close()
            self.assertIsNotNone(row)
            self.assertEqual(str(row[0]), "cancelled")
            self.assertEqual(str(row[1]), "autofix_superseded_by_newer_terminal")
            self.assertIsNotNone(followup)
            self.assertEqual(str(followup[0]), "architect_local")
            self.assertEqual(str(followup[1]), "queued")

    def test_unknown_blocker_uses_faster_stale_threshold(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = Path(td) / "jobs.sqlite"
            conn = sqlite3.connect(str(db_path))
            conn.row_factory = sqlite3.Row
            ...
            conn = sqlite3.connect(str(db_path))
            row = conn.execute("SELECT state, blocked_reason FROM jobs WHERE job_id='blocked-fast-unknown'").fetchone()
            conn.close()
            self.assertIsNotNone(row)
            self.assertEqual(str(row[0]), "cancelled")
            self.assertEqual(str(row[1]), "autofix_stale_blocked_only")

    def test_enqueues_blocker_validity_review_when_blocked_only_is_stuck_but_not_cleanup_eligible(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = Path(td) / "jobs.sqlite"
            conn = sqlite3.connect(str(db_path))
            conn.row_factory = sqlite3.Row
            ...
            # Blocked only, age>300, structured waiting job reason but dependency missing:
            # not superseded, not stale (threshold 900), should enqueue reviewer_local.
            conn.execute(
                """
                INSERT INTO jobs(
                    job_id, source, role, input_text, request_type, priority, model, effort, mode_hint,
                ...
                (
                    ...
                    0,
                    None,
                    "waiting_for_job:missing-dep-job",
                    0,
                    None,
                    "{}",
                ),
            ...
            conn = sqlite3.connect(str(db_path))
            blocked = conn.execute("SELECT state FROM jobs WHERE job_id='blocked-review-needed'").fetchone()
            review = conn.execute(
                "SELECT role, state FROM jobs WHERE parent_job_id=? AND role='reviewer_local' ORDER BY created_at DESC LIMIT 1",
                (order_id,),
            ).fetchone()
            conn.close()
            self.assertIsNotNone(blocked)
            self.assertEqual(str(blocked[0]), "blocked")
            self.assertIsNotNone(review)
            self.assertEqual(str(review[0]), "reviewer_local")
            self.assertEqual(str(review[1]), "queued")
