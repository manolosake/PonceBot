...
class SQLiteTaskStorage:
        ...
        with self._conn() as conn:
            ...
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS agent_runtime_state (
                    ...
                    lane TEXT NOT NULL DEFAULT 'factory',
                    heartbeat_at REAL,
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_repo_registry_status ON repo_registry(status, autonomy_enabled, priority, updated_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_repo_registry_path ON repo_registry(path)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_agent_runtime_repo_role ON agent_runtime_state(repo_id, role)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_agent_runtime_heartbeat ON agent_runtime_state(heartbeat_at DESC)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_agent_mailbox_target_status ON agent_mailbox(to_agent_key, status, created_at DESC)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_agent_mailbox_repo_status ON agent_mailbox(repo_id, status, created_at DESC)")
    ...
    def upsert_order(
        ...
        *,
        ...
        if ph not in ("planning", "delegated", "executing", "review", "ceo_approval_pending", "done", "paused"):
            ph = "planning"
    ...
    def upsert_repo(
        ...
        *,
        ...
        branch = (default_branch or "").strip() or "main"
        runtime = (runtime_mode or "").strip().lower() or "ceo-bounded"
        repo_status = (status or "").strip().lower() or "active"
        try:
            prio = int(priority)
        except Exception:
            prio = 2
        ...
        try:
            budget = float(daily_budget or 0.0)
        except Exception:
            budget = 0.0
        if budget < 0:
            budget = 0.0
        ...
        with self._lock:
            with self._conn() as conn:
                now = float(time.time())
                existing = conn.execute(
                    "SELECT created_at FROM repo_registry WHERE repo_id = ?",
                    (rid,),
                ).fetchone()
                created_at = float(existing["created_at"]) if existing is not None else now
                conn.execute(
                    "INSERT OR REPLACE INTO repo_registry(repo_id, path, default_branch, autonomy_enabled, priority, runtime_mode, daily_budget, status, metadata, created_at, updated_at) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        rid,
                        repo_path,
                        branch,
                        1 if autonomy_enabled else 0,
                        int(prio),
                        runtime,
                        float(budget),
                        repo_status,
    ...
    def get_repo(self, *, repo_id: str) -> dict[str, Any] | None:
        ...
        with self._conn() as conn:
            row = conn.execute(
                "SELECT repo_id, path, default_branch, autonomy_enabled, priority, runtime_mode, daily_budget, status, metadata, created_at, updated_at "
            ...
            return {
                ...
                "autonomy_enabled": bool(_coerce_bool(row["autonomy_enabled"], True)),
                "priority": int(_coerce_int(row["priority"], 2) or 2),
                "runtime_mode": str(row["runtime_mode"]),
                "daily_budget": float(_coerce_float(row["daily_budget"], 0.0) or 0.0),
                "status": str(row["status"]),
                "metadata": _coerce_json_dict(row["metadata"]),
    ...
    def get_repo_by_path(self, *, path: str) -> dict[str, Any] | None:
        ...
        with self._conn() as conn:
            row = conn.execute(
                "SELECT repo_id FROM repo_registry WHERE path = ?",
                (repo_path,),
            ).fetchone()
            if row is None:
                return None
        return self.get_repo(repo_id=str(row["repo_id"]))
    ...
    def list_repos(
        ...
        *,
        ...
        repo_status = (status or "").strip().lower() or None
        lim = max(1, min(5000, int(limit)))
        where: list[str] = []
        params: list[Any] = []
        if repo_status:
            where.append("status = ?")
            params.append(repo_status)
        if autonomy_enabled is not None:
            where.append("autonomy_enabled = ?")
            params.append(1 if autonomy_enabled else 0)
        clause = (" WHERE " + " AND ".join(where)) if where else ""
        with self._conn() as conn:
            rows = conn.execute(
                "SELECT repo_id, path, default_branch, autonomy_enabled, priority, runtime_mode, daily_budget, status, metadata, created_at, updated_at "
                f"FROM repo_registry{clause} ORDER BY priority ASC, updated_at DESC LIMIT ?",
                [*params, lim],
            ).fetchall()
            out: list[dict[str, Any]] = []
            for row in rows:
    ...
    def set_repo_autonomy(self, *, repo_id: str, enabled: bool) -> bool:
        ...
        with self._lock:
            with self._conn() as conn:
                cur = conn.execute(
                    "UPDATE repo_registry SET autonomy_enabled = ?, updated_at = ? WHERE repo_id = ?",
                    (1 if enabled else 0, float(time.time()), rid),
                )
                conn.commit()
    ...
    def set_repo_status(self, *, repo_id: str, status: str) -> bool:
        rid = (repo_id or "").strip()
        repo_status = (status or "").strip().lower()
        if not rid or not repo_status:
            return False
        with self._lock:
            with self._conn() as conn:
                cur = conn.execute(
                    "UPDATE repo_registry SET status = ?, updated_at = ? WHERE repo_id = ?",
                    (repo_status, float(time.time()), rid),
                )
                conn.commit()
                return bool(cur.rowcount)
    ...
    def upsert_agent_runtime_state(
        ...
        *,
        ...
        rid = (repo_id or "").strip()
        agent_role = (role or "").strip().lower()
        if not rid:
            raise ValueError("repo_id required")
        if not agent_role:
            raise ValueError("role required")
        agent_key = f"{rid}:{agent_role}"
        lane_name = (lane or "").strip().lower() or "factory"
        summary = str(last_result_summary or "").strip()
        if len(summary) > 4000:
        ...
        with self._lock:
            with self._conn() as conn:
                now = float(time.time())
                existing = conn.execute(
                    "SELECT metadata, session_thread_id, chat_id, heartbeat_at, last_result_status, last_result_summary, last_job_id, last_run_at, lane "
                    "FROM agent_runtime_state WHERE agent_key = ?",
                    (agent_key,),
                ).fetchone()
                existing_meta = _coerce_json_dict(existing["metadata"]) if existing is not None else {}
                merged_meta = dict(existing_meta)
                if metadata:
                    merged_meta.update(metadata)
                conn.execute(
                    "INSERT OR REPLACE INTO agent_runtime_state(agent_key, repo_id, role, chat_id, session_thread_id, lane, heartbeat_at, last_result_status, last_result_summary, last_job_id, last_run_at, metadata, updated_at) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        agent_key,
                        rid,
                        agent_role,
                        (int(chat_id) if chat_id is not None else (_coerce_int(existing["chat_id"], None) if existing is not None else None)),
                        str(session_thread_id or "").strip() or (str(existing["session_thread_id"]) if existing is not None and existing["session_thread_id"] is not None else None),
                        lane_name or (str(existing["lane"]) if existing is not None else "factory"),
                        (float(heartbeat_at) if heartbeat_at is not None else (_coerce_float(existing["heartbeat_at"], None) if existing is not None else None)),
                        (str(last_result_status or "").strip() or (str(existing["last_result_status"]) if existing is not None and existing["last_result_status"] is not None else None)),
                        summary or (str(existing["last_result_summary"]) if existing is not None and existing["last_result_summary"] is not None else None),
                        str(last_job_id or "").strip() or (str(existing["last_job_id"]) if existing is not None and existing["last_job_id"] is not None else None),
                        (float(last_run_at) if last_run_at is not None else (_coerce_float(existing["last_run_at"], None) if existing is not None else None)),
    ...
    def get_agent_runtime_state(self, *, repo_id: str, role: str) -> dict[str, Any] | None:
        rid = (repo_id or "").strip()
        agent_role = (role or "").strip().lower()
        if not rid or not agent_role:
            return None
        agent_key = f"{rid}:{agent_role}"
        with self._conn() as conn:
            row = conn.execute(
                "SELECT agent_key, repo_id, role, chat_id, session_thread_id, lane, heartbeat_at, last_result_status, last_result_summary, last_job_id, last_run_at, metadata, updated_at "
                "FROM agent_runtime_state WHERE agent_key = ?",
                (agent_key,),
            ).fetchone()
            if row is None:
            ...
            return {
                ...
                "repo_id": str(row["repo_id"]),
                "role": str(row["role"]),
                "chat_id": _coerce_int(row["chat_id"], None),
                "session_thread_id": (str(row["session_thread_id"]) if row["session_thread_id"] is not None else None),
                "lane": str(row["lane"]),
                "heartbeat_at": _coerce_float(row["heartbeat_at"], None),
                "last_result_status": (str(row["last_result_status"]) if row["last_result_status"] is not None else None),
                "last_result_summary": (str(row["last_result_summary"]) if row["last_result_summary"] is not None else None),
                "last_job_id": (str(row["last_job_id"]) if row["last_job_id"] is not None else None),
                "last_run_at": _coerce_float(row["last_run_at"], None),
                "metadata": _coerce_json_dict(row["metadata"]),
                "updated_at": float(row["updated_at"]),
            }
    ...
    def list_agent_runtime_state(
        ...
        *,
        ...
        rid = (repo_id or "").strip() or None
        agent_role = (role or "").strip().lower() or None
        ...
        if stale_before is not None:
            where.append("(heartbeat_at IS NULL OR heartbeat_at <= ?)")
            params.append(float(stale_before))
        clause = (" WHERE " + " AND ".join(where)) if where else ""
        with self._conn() as conn:
            rows = conn.execute(
                "SELECT agent_key, repo_id, role, chat_id, session_thread_id, lane, heartbeat_at, last_result_status, last_result_summary, last_job_id, last_run_at, metadata, updated_at "
                f"FROM agent_runtime_state{clause} ORDER BY repo_id ASC, role ASC LIMIT ?",
                [*params, lim],
            ).fetchall()
            out: list[dict[str, Any]] = []
            for row in rows:
    ...
    def append_agent_mailbox(
        ...
        *,
        ...
        target_key = (to_agent_key or "").strip()
        msg_kind = (kind or "").strip().lower()
        if not target_key:
            raise ValueError("to_agent_key required")
        if not msg_kind:
            raise ValueError("kind required")
        mid = (message_id or "").strip() or str(uuid.uuid4())
        repo = (repo_id or "").strip() or None
        source_key = (from_agent_key or "").strip() or None
        corr = (correlation_id or "").strip() or None
        msg_status = (status or "").strip().lower() or "queued"
        with self._lock:
            with self._conn() as conn:
                now = float(time.time())
                conn.execute(
                    "INSERT OR REPLACE INTO agent_mailbox(message_id, repo_id, from_agent_key, to_agent_key, kind, correlation_id, payload, status, created_at, updated_at) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        mid,
                        repo,
    ...
    def list_agent_mailbox(
        ...
        *,
        ...
        target_key = (to_agent_key or "").strip() or None
        repo = (repo_id or "").strip() or None
        msg_status = (status or "").strip().lower() or None
        lim = max(1, min(5000, int(limit)))
        where: list[str] = []
        params: list[Any] = []
        if target_key:
            where.append("to_agent_key = ?")
        ...
        if msg_status:
            where.append("status = ?")
            params.append(msg_status)
        clause = (" WHERE " + " AND ".join(where)) if where else ""
        with self._conn() as conn:
            rows = conn.execute(
                "SELECT message_id, repo_id, from_agent_key, to_agent_key, kind, correlation_id, payload, status, created_at, updated_at "
                f"FROM agent_mailbox{clause} ORDER BY created_at DESC LIMIT ?",
                [*params, lim],
            ).fetchall()
            out: list[dict[str, Any]] = []
            for row in rows:
                out.append(
                    {
                        ...
                        "from_agent_key": (str(row["from_agent_key"]) if row["from_agent_key"] is not None else None),
    ...
    def set_agent_mailbox_status(self, *, message_id: str, status: str) -> bool:
        ...
        msg_status = (status or "").strip().lower()
        if not mid or not msg_status:
            return False
        with self._lock:
            with self._conn() as conn:
                cur = conn.execute(
                    "UPDATE agent_mailbox SET status = ?, updated_at = ? WHERE message_id = ?",
                    (msg_status, float(time.time()), mid),
                )
                conn.commit()
                return bool(cur.rowcount)
