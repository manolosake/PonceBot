...
class StatusAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path in ("/api/v1/orchestration/overview", "/api/orchestration/overview"):
            ...
            self._send_json(
                200,
                {
                    ...
                    "repos_count": len(list(snap.get("repos") or [])),
        ...
        if path in ("/api/v1/orchestration/proactive-health", "/api/orchestration/proactive-health"):
            if not self.server.allow_snapshot(ip):
                self._send_json(429, {"error": "rate_limited"}, extra_headers={"Retry-After": "1"})
                return
        ...
        if path in ("/api/v1/orchestration/proactive-action-plan", "/api/orchestration/proactive-action-plan"):
            if not self.server.allow_snapshot(ip):
                self._send_json(429, {"error": "rate_limited"}, extra_headers={"Retry-After": "1"})
                return
            limit = _parse_limit(qs, 20, hi=200)
        ...
        if path in (
            "/api/v1/orchestration/operator-focus/digest",
            "/api/orchestration/operator-focus/digest",
        ):
            if not self.server.allow_snapshot(ip):
                self._send_json(429, {"error": "rate_limited"}, extra_headers={"Retry-After": "1"})
                return
            limit = _parse_limit(qs, 5, hi=20)
            payload = self.server.status_service.operator_focus_digest(
                chat_id=chat_id,
                limit=limit,
                categories=_parse_filter_values(qs, "category"),
                urgencies=_parse_filter_values(qs, "urgency"),
        ...
        if path in (
            "/api/v1/orchestration/operator-focus/shift-brief",
            "/api/orchestration/operator-focus/shift-brief",
        ):
            if not self.server.allow_snapshot(ip):
                self._send_json(429, {"error": "rate_limited"}, extra_headers={"Retry-After": "1"})
                return
        ...
        if path in (
            "/api/v1/orchestration/operator-focus/receipt-trail.md",
            "/api/orchestration/operator-focus/receipt-trail.md",
        ):
            if not self.server.allow_snapshot(ip):
                self._send_json(429, {"error": "rate_limited"}, extra_headers={"Retry-After": "1"})
                return
            ...
            if not action_id and (not rank_ok or rank is None):
                self._send_json(400, {"error": "invalid_rank", "selection": {"rank": (qs.get("rank") or [""])[0].strip()}})
                return
            limit = _parse_limit(qs, 20, hi=100)
            payload = self.server.status_service.operator_focus_receipt_trail(
                chat_id=chat_id,
                action_id=action_id or None,
                rank=rank,
                categories=_parse_filter_values(qs, "category"),
            ...
            if not payload.get("item_identity"):
                self._send_markdown(404, render_receipt_trail_markdown(payload))
                return
            self._send_markdown(200, render_receipt_trail_markdown(payload))
            return

        if path in (
            "/api/v1/orchestration/operator-focus/receipt-trail",
            "/api/orchestration/operator-focus/receipt-trail",
        ):
            if not self.server.allow_snapshot(ip):
                self._send_json(429, {"error": "rate_limited"}, extra_headers={"Retry-After": "1"})
                return
        ...
        if path in (
            "/api/v1/orchestration/orders/handoff-digest",
            "/api/orchestration/orders/handoff-digest",
        ):
            if not self.server.allow_snapshot(ip):
                self._send_json(429, {"error": "rate_limited"}, extra_headers={"Retry-After": "1"})
                return
            ...
            child_limit = _parse_named_limit(qs, "child_limit", 200, hi=2000)
            log_limit = _parse_named_limit(qs, "log_limit", 200, hi=2000)
            digest = self.server.status_service.order_handoff_digest(
                order_id,
                trace_limit=trace_limit,
                child_limit=child_limit,
                log_limit=log_limit,
