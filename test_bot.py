...
class TestStudioOutcomeMemory(unittest.TestCase):
    ...
    def test_publication_contract_reconcile_confirms_private_github_visibility_from_api(self) -> None:
        def run(cmd: list[str], cwd: Path) -> str:
            real_git = os.environ.get("PONCEBOT_REAL_GIT") or shutil.which("git") or "git"
            if cmd and cmd[0] == "git":
                cmd = [real_git, *cmd[1:]]
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.object(bot, "_github_repo_private_visibility", return_value=True) as private_visibility:
                changed = bot._studio_reconcile_portfolio_publication_contract(db, now=now + 60)
            private_visibility.assert_called_once_with("manolosake/permit-sprint")
            with sqlite3.connect(db) as conn:
                conn.row_factory = sqlite3.Row
                project_row = conn.execute(
        ...
        self.assertEqual(project_row["latest_head"], head)
        self.assertEqual(project_row["latest_summary"], "Published repo needs verification.")
        self.assertIsNotNone(recovery_row)
        self.assertEqual(recovery_row["status"], "resolved")
        self.assertEqual(recovery_row["missing_json"], "[]")
        self.assertIn("Publication contract is complete", recovery_row["reason"])

    def test_publication_contract_reconcile_keeps_repo_url_mismatch_open(self) -> None:
        def run(cmd: list[str], cwd: Path) -> str:
            real_git = os.environ.get("PONCEBOT_REAL_GIT") or shutil.which("git") or "git"
            if cmd and cmd[0] == "git":
                cmd = [real_git, *cmd[1:]]
