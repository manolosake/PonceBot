...
def _extract_declared_paths(payload: dict[str, Any]) -> list[str]:
    out: list[str] = []
    for item in payload.get("declared_files", []):
...
def evaluate_bundle(
    ...
    *,
    ...
    patch_apply_check = artifacts_dir / "patch_apply_check.json"
    ...
    checks["patch_apply_check_exists"] = patch_apply_check.exists()
    ...
    if not checks["patch_apply_check_exists"]:
        errors.append("missing patch_apply_check.json")
    ...
    apply_payload = json.loads(patch_apply_check.read_text(encoding="utf-8", errors="replace"))
    declared_paths = _extract_declared_paths(apply_payload if isinstance(apply_payload, dict) else {})
    ...
    missing_declared = sorted(p for p in declared_paths if p not in patch_paths)
    checks["declared_paths_covered_by_patch"] = len(missing_declared) == 0
    if missing_declared:
        errors.append("patch_apply_check declared files missing in patch")

    checks["changes_patch_non_empty_when_declared"] = not declared_paths or checks["changes_patch_non_empty"]
    checks["git_status_non_empty_when_declared"] = not declared_paths or checks["git_status_non_empty"]
    if declared_paths and not checks["changes_patch_non_empty"]:
        errors.append("changes.patch is empty while patch_apply_check declares files")
    if declared_paths and not checks["git_status_non_empty"]:
        errors.append("git_status.txt is empty while patch_apply_check declares files")
    ...
    mismatches.extend(
        {"type": "declared_missing_in_patch", "path": p}
        for p in missing_declared
    )
